...
def get_all_prefixes(path):
    ...
    Generate versions both with and without indexes
    """
...
def get_meaningful_ie_paths(flattened_records):
    ...
    for record in flattened_records:
        ...
        # 3. Add intermediate paragraph combinations
        segments = parse_path_segments(path)
...
def path_contains_substring(path, substring):
    """Check if path contains a substring"""
...
def split_into_continuous_groups(records):
    ...
    Core improvement: Ensure that field_id within each IE is consecutive
    """
    if not records:
        return []
...
def extract_ies(input_file, output_dir, min_fields=1, max_fields=100, ensure_continuous=False):
    """
    Main function: Extract all IEs and save to file
    ...
    Args:
        ...
        ensure_continuous: Whether to ensure field_id is continuous (⭐new parameter)
    """
    ...
    # For each IE path, collect matching records
    ...
    print(f"After initial screening, {len(ie_map)} IEs remain")
    ...
    if ensure_continuous:
        ...
        print(f"- Remaining after continuous screening: {len(continuous_ie_map)} IEs")
        ie_map = continuous_ie_map
    else:
        # Filter by field count only
        ie_map = {k: v for k, v in ie_map.items() 
                  if min_fields <= len(v) <= max_fields}
        print(f"{len(ie_map)} IEs remaining after field count filtering")
    ...
    print(f"- Number of IE files saved: {saved_count}")
    print(f"- Field count range: {min_fields}-{max_fields}")
    print(f"  - continuous requirement: {'Yes' if ensure_continuous else 'No'}")
    print(f"- File name conflicts: {duplicate_count}")
...
def main():
    ...
    min_fields = 1       # ← Contains single-field IE
    max_fields = 100     # ← More relaxed
    ensure_continuous = False  # ← No continuity required
