...
def extract_ie_name_variations(filename):
    """
    Extract all possible variants of IE names from file names
    ...
    if '_' in name:
        ...
        variations.append(last_part)  # Final part
        # Also tried removing the numeric suffix
    ...
    return list(set(variations))  # Deduplication
...
def find_matching_asn1_file(ie_name_variations, asn1_files):
    ...
    Returns (matched ASN.1 filename, IE name variant used), or (None, None) if no match is found
    """
...
def get_field_id_range(filepath):
    """
    Extract field_id range from IE file
...
def filter_ies_with_asn1_rules(verbose=True):
            ...
            if field_range:
                ...
                if field_range in field_range_map:
                    ...
                    if current_score > existing_score:
                        ...
                        # Keep original files
                        if verbose:
                            print(f"⚠ Deduplication: Skipping {ie_file} (already exists {existing_file}, same range: {field_range})")
                        continue
                else:
                    # New field scope, add
                    field_range_map[field_range] = {
    ...
    print(f"- Deduplicated IEs saved: {final_count}")
    ...
    if unmatched_ies:
        ...
        for ie_file, variations in unmatched_ies[:20]:
            print(f"  - {ie_file}")
            print(f"Attempted variations: {variations}")
