                    ...
                    slot_t slot_number)
{
  ...
  // Validate startPosition and nrofSymbols relationship
  // if (l_offset < (N_symb_SRS - 1)) {
  //   LOG_E(NR_PHY, "generate_srs: inconsistent parameters! time_start_position (%d) must be >= num_symbols-1 (%d)\n", 
  //         l_offset, N_symb_SRS - 1);
  //   return -1;
  // }
  ...
  for (int p_index = 0; p_index < N_ap; p_index++) {
    ...
    for (int l_line = 0; l_line < N_symb_SRS; l_line++) {
      // uint8_t current_symbol = l0 + l_line;
      // if (current_symbol >= frame_parms->symbols_per_slot) {
      //   LOG_E(NR_PHY, "generate_srs: SRS symbol %d (l0=%d + l_line=%d) exceeds slot boundary %d\n", 
      //     current_symbol, l0, l_line, frame_parms->symbols_per_slot - 1);
      //   return -1;
      // }
      ...
      for (int k = 0; k < M_sc_b_SRS; k++) {
        ...
        uint32_t write_index = symbol_offset + l_line_offset + subcarrier;
        uint32_t max_index = frame_parms->symbols_per_slot * frame_parms->ofdm_symbol_size;
        
        if (write_index >= max_index) {
            LOG_E(NR_PHY, "generate_srs: Buffer overflow prevented! write_index=%u >= max=%u (symbol_offset=%u, l_line_offset=%u, subcarrier=%u)\n",
                  write_index, max_index, symbol_offset, l_line_offset, subcarrier);
            continue;  // 或者 continue; 跳过这次写入
        }
