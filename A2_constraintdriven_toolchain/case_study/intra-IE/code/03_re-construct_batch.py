...
FLATTENED_DIR = "../output/startposition_nrofsymbols"
...
def overlay_flattened(skeleton, flattened_records, log_file):
    ...
    for idx, record in enumerate(flattened_records):
        ...
        if val is None:
            log_both(log_file, f"  ⚠️  记录 # {idx}: The value of {field_path} is None (possibly normal)")
...
def process_file(flattened_file, metadata, log_file):
    ...
    try:
        ...
        if errors:
            log_both(log_file, f"\n  ⚠️  {len(errors)} errors occurred during the override process:")
