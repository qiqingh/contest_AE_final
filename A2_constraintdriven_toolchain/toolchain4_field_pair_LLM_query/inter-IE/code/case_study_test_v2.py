...
def main():
    ...
    # 使用改进后的采样函数（与批量脚本一致）
    print(f"   Applying IMPROVED sampling strategy...")
    ...
    # IMPROVED: More precise keyword patterns for inter-IE constraints
    # Use complete phrases and combinations for better discrimination
    inter_ie_patterns = [
        # CRITICAL phrases (40 points each) - MUST rank at top
        ('association between', 40),  # CRITICAL: Significantly increased to ensure top ranking
        ('reference between', 40),
        
        # High-value phrases (15 points each) - strong indicators of inter-IE relationships
        ('reference to', 15),
        ('refers to', 15),
        ('shall match', 20),
        
        # Medium-value phrases (3-5 points each) - moderate indicators
        ('match', 3),  # Minimized to reduce false positives
        ('associated with', 5),
        ('referenced by', 5),
        ('linked to', 5),
        ('corresponds to', 5),
    ...
    def calculate_relevance_score(evidence):
        """Calculate comprehensive relevance score with improved keyword matching"""
        ...
        # 3. IMPROVED keyword/pattern matching
        text = evidence.get('text', '').lower()
        keyword_score = 0
        
        # Match precise patterns
        for pattern, points in inter_ie_patterns:
            if pattern in text:
                keyword_score += points
        
        # Bonus: Check for field name combinations (strong signal for inter-IE)
        field_mentions = 0
        for field_name in ['controlresourcesetid', 'control resource set', 'coreset']:
            if field_name in text:
                field_mentions += 1
        if field_mentions >= 2:
            keyword_score += 10  # Strong signal that this discusses the relationship
        
        total_score = confidence_score + section_score + keyword_score
        return total_score
    
    # Sort by comprehensive relevance score
    high_quality_evidences.sort(key=calculate_relevance_score, reverse=True)
    
    # Show top 5 scores for better debugging
    print(f"   Top 5 relevance scores (improved algorithm):")
    for idx, e in enumerate(high_quality_evidences[:5], 1):
        score = calculate_relevance_score(e)
        conf = e.get('confidence', 'UNKNOWN')
        section_score = e.get('section_relevance', {}).get('combined_score', 0)
        preview = e['text'][:60]
        print(f"     {idx}. Total={score:.0f} (conf={conf}:40, section={section_score}:20): {preview}...")
    
    # Take top 10
    high_quality_evidences = high_quality_evidences[:10]
    print(f"   ✅ Using top 10 by improved relevance scoring")
    ...
    if "DSL" in result_field and has_dsl_content:
        ...
        if result_field != "DSL":
            print(f"   ⚠️  Note: result field was '{api_response.get('result')}' (non-standard format)")
    else:
        print(f"⭕ NO_RULE")
