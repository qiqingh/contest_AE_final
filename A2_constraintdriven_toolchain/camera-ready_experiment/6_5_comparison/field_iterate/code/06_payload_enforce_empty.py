...
input_dir = '../output/calOffset'
# output_dir = '../output/boundary_test_exploits'
output_dir = '../output/payloads'
diagnosis_dir = './diagnosis/diagnosis_payloads.txt'
...
"""
    ...
    # 只有在有modifications时才添加修改代码
    if modifications:
        for offset, new_value in modifications:
            exploit_code += f"        pkt_buf[{offset} - 48] = 0x{new_value};\n"
    else:
        # 可选：为空文件添加一个注释
        exploit_code += "        // No modifications for this exploit\n"
...
"""
    ...
    try:
        ...
        if modifications:
            log_message(f"Successfully generated exploit: {exploit_name} with {len(modifications)} modifications from {source_file}")
        else:
            log_message(f"Successfully generated empty exploit: {exploit_name} (no modifications) from {source_file}")
        return True
    except Exception as e:
        error_msg = f"ERROR: Failed to write exploit file {exploit_name}: {str(e)}"
        log_message(error_msg)
...
def main():
    ...
    for filename in txt_files:
        ...
        file_size = os.path.getsize(file_path)
        if file_size == 0:
            log_message(f"注意: 文件 {filename} 为空 (0字节)，生成不含修改的exploit")
            empty_files += 1
            # 为空文件生成exploit
            exploit_name = filename.replace('.txt', '.cpp')
            if generate_exploit_file([], exploit_name, filename):
                success_count += 1
            else:
                error_count += 1
            continue
        ...
        # 即使没有找到有效的修改，也生成文件
        if not modifications:
            log_message(f"文件 {filename} 中没有找到有效的修改指令，生成空exploit")
...
- 总共找到 {len(txt_files)} 个.txt文件
- 成功生成 {success_count} 个exploit文件
- 生成失败 {error_count} 个文件
- 空文件 {empty_files} 个（已生成对应的空exploit）
"""
    log_message(summary)
