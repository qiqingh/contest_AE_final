...
#     """
#     为INTEGER类型字段生成变异值，重点测试边界值。
    
#     Args:
#         field_info: 字段信息
#         num_mutations: 生成的变异数量（在这个版本中忽略，始终返回所有边界值）
...
#                 "mutation_type": "min_plus_one",
#                 "description": f"最小值加1 ({min_val+1})"
#             })
...
#                 "mutation_type": "max_minus_one",
#                 "description": f"最大值减1 ({max_val-1})"
#             })
        
#         # 如果类型允许，添加最小值-1 (超出下界)
#         if min_val > -2**31:  # 假设INTEGER类型最小支持32位有符号整数
#             mutations.append({
#                 "mutation_value": min_val - 1,
#                 "mutation_type": "min_minus_one",
#                 "description": f"最小值减1 ({min_val-1}) - 超出下界"
#             })
        
#         # 如果类型允许，添加最大值+1 (超出上界)
#         if max_val < 2**31 - 1:  # 假设INTEGER类型最大支持32位有符号整数
#             mutations.append({
#                 "mutation_value": max_val + 1,
#                 "mutation_type": "max_plus_one",
#                 "description": f"最大值加1 ({max_val+1}) - 超出上界"
#             })
...
#         return [
#             {
#                 "mutation_value": field_info["asn1_rules"]["rules"][0]["range"][0],  # 最小值
#                 "mutation_type": "min_value",
#                 "description": "最小有效值 (超时简化)"
#             },
#             {
#                 "mutation_value": field_info["asn1_rules"]["rules"][0]["range"][1],  # 最大值
#                 "mutation_type": "max_value",
#                 "description": "最大有效值 (超时简化)"
#             }
#         ]
...
#         return []
#     finally:
#         signal.alarm(0)  # 确保超时被取消

def generate_integer_mutations(field_info: Dict[str, Any], num_mutations: int) -> List[Dict[str, Any]]:
    """
    为INTEGER类型字段生成变异值，测试关键边界值。
    ...
    try:
        ...
        if max_val - min_val > 2:
            ...
            # 如果范围只有3个值，中间值就是min_val+1，已经在上面添加过了
            pass
                ...
                for i, mutation in enumerate(filtered_mutations, 1):
                    try:
                        if create_mutation_file(field_id, mutation, original_data, output_dir, i):
                            mutations_count += 1
                            print(f"  已创建变异: {filename} -> 变异 {i}/{len(filtered_mutations)} ({mutation['description']})")
                    except Exception as e:
                        print(f"  创建变异文件时出错: {e}")
...
def main():
    """主函数"""
    # 设置目录
    input_dir = "/home/qiqingh/Desktop/usenix_security_rebuttal_prepare/rebuttal_comparison_experiment/single_field/asn-update/combine_fields"
    original_file_path = "/home/qiqingh/Desktop/usenix_security_rebuttal_prepare/rebuttal_comparison_experiment/single_field/asn-update/02_flatten.json"
    output_dir = "../output/single_field_mutations_asn"
