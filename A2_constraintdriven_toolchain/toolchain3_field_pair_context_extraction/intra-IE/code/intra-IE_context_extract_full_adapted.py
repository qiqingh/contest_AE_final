...
# -*- coding: utf-8 -*-
"""
Intra-IE Context Extraction Tool - Full Featured Adapted Version
Full-featured adapted version - retains all optimizations from the original, adapted to field list format
"""
...
GENERATE_ALL_PAIRS = True  # True=All combinations, False=Adjacent pairs
...
# ============================================================================
# Core Extractor Class (retains all original functionality)
# ============================================================================
...
class EnhancedConstraintExtractor:
    def __init__(self, debug=False, strict_mode=True):
        ...
        # 5G Protocol Terminology Abbreviation Mapping Table (Original Version Fully Retained)
        ...
        # Restricted Keywords (Original Complete List)
        ...
        # Self-referencing pattern keywords
    ...
    # ==================== LaTeX Cleanup (Original Version Retained) ====================
    ...
    # ==================== IE Information Extraction (New Field List Support) ====================
    
    def extract_ie_name_from_path(self, field_path: str) -> str:
        """Extract IE name from field_path"""
    ...
    def extract_ie_info_from_filename(self, json_path: str) -> Dict:
        """Extract IE information from filename (compatible with original interface)"""
    ...
    def generate_ie_variants(self, ie_info: Dict, field_names: List[str] = None) -> List[str]:
        """Generate IE Name Variants (Enhanced: Including Field Name Variants)"""
        variants = set()
    ...
    # ==================== Chapter Index Construction (Original Version Retained) ====================
    ...
    # ==================== Field Processing (Original Version Fully Preserved) ====================
    
    def load_field_names_from_list(self, field_list: List[Dict]) -> Tuple[List[str], Dict[str, List[int]]]:
        """Load field names from field list (adapted for new format)"""
        field_names = set()
        field_id_mapping = {}
    ...
    def normalize_field_name(self, field_name: str) -> str:
        """Standardized field names (cached)"""
    ...
    def get_acronym_patterns(self, field_name: str) -> List[str]:
        """Generate abbreviation matching patterns"""
        acronym_patterns = []
    ...
    def generate_abbreviations(self, field_name: str) -> List[str]:
        """Generate field abbreviations (cache)"""
        abbreviations = []
    ...
    def is_known_abbreviation(self, field_name: str, matched_text: str) -> bool:
        """Check if it is a known abbreviation"""
    ...
    def contains_constraint_keyword_fast(self, sentence: str) -> Tuple[bool, str]:
        """Quick check constraint keywords"""
    ...
    def contains_self_reference_keywords(self, text: str, field_name: str) -> bool:
        """Detect self-referencing keywords"""
    ...
    def get_context_lines(self, lines: List[str], line_idx: int, context_size: int = 2) -> List[str]:
        """Get context lines"""
    ...
    def batch_find_field_occurrences(self, lines: List[str], field_patterns: Dict[str, Dict]) -> Dict[str, List[Tuple[int, str]]]:
        """Find field occurrence positions in batch"""
    ...
    def extract_constraints_from_file_optimized(self, file_path: str, field_pairs: List[Dict], 
        ...
        """Optimized Constraint Extraction"""
        constraints = []
        ...
        # Relevance threshold: 0.0 means retaining all constraints (including LOW constraints with 0<score<5)
        # The original value of 5.0 would lose these potentially valuable constraints
            ...
            if is_self_pair and include_self_constraints:
                ...
                for line_idx, match_text in field_occurrences[field1]:
                    ...
                    if len(matches) >= 2 or self.contains_self_reference_keywords(clean_line, field1):
                        ...
                        if has_constraint:
                            context_lines = self.get_context_lines(lines, line_idx, context_size=2)
                    ...
                    for line2_idx, match2_text in field_occurrences[field2]:
                        if abs(line1_idx - line2_idx) <= 1:
                            if line1_idx == line2_idx:
                                text = lines[line1_idx].strip()
                            else:
                                min_idx = min(line1_idx, line2_idx)
                                max_idx = max(line1_idx, line2_idx)
                                text = lines[min_idx].strip() + ' ' + lines[max_idx].strip()
...
# ============================================================================
# Processing function (adapted field list format)
# ============================================================================
...
def process_single_ie_adapted(ie_json_path: str, spec_dir: str, output_dir: str,
                              ...
                              use_all_pairs=False):
    """Process single IE (adapt field list format)"""
    ...
    try:
        ...
        # 5. Pre-computed Field Pattern
        field_patterns = extractor.precompute_field_patterns(field_names)
        
        # 6. Determine regulatory documents
        # Extract from field_path or use default specification
        field_paths = [item.get('field_path', '') for item in field_list if 'field_path' in item]
        
        if field_paths:
            ie_extracted = extractor.extract_ie_name_from_path(field_paths[0])
            # Infer specification number from filename
            match = re.match(r'(\d+)_', os.path.basename(ie_json_path))
            if match:
                ...
                spec_number = spec_mapping.get(spec_series, f'38.{spec_series}')
            else:
                spec_number = '38.331'
        else:
            spec_number = '38.331'
        ...
        spec_file = None
        for ext in ['.md', '.txt']:
            for name in [f"{spec_number}{ext}", f"{spec_number.replace('.', '')}{ext}"]:
        ...
        if not spec_file:
            # Use all specification files in the directory
            spec_files = []
            for ext in ['*.txt', '*.md']:
                spec_files.extend([f for f in Path(spec_dir).glob(ext)])
        else:
...
def batch_process_ies_adapted(ie_dir: str, spec_dir: str, output_dir: str,
                              ...
                              use_multiprocessing=True, include_self_constraints=True,
                              use_all_pairs=False):
    """Batch Process IE (Compatible Version)"""
    ...
    print(f"\n{'='*60}")
    print(f"Batch Processing {len(ie_files)} IE files (Adapted Version)")
...
def main():
    parser = argparse.ArgumentParser(
        description='Enhanced Intra-IE Constraint Extractor (Adapted for Field List Format)'
    )
