...
def main():
    ...
    if not os.path.exists(INTER_IE_DIR):
        print(f"❌ Error: Inter-IE directory does not exist")
    ...
    print(f"Core cross-cluster pairs:")
    ...
    if ENABLE_EXTENDED_PAIRS:
        print(f"Expand cross-cluster pairs:")
    ...
    allowed_intra = [] if DISABLE_ALL_INTRA_CLUSTER else ["C7_MAC"]  # Keep only the smallest
    ...
    if skipped_pairs:
        print(f"Skipped oversized cluster pairs:")
        skipped_total = 0
        for c1, c2, count, reason in skipped_pairs:
            skipped_total += count
            print(f"{c1:20s} × {c2:20s}: {count:5d} pairs ({reason})")
        print(f"Saved: {skipped_total:,} pairs")
    ...
    print(f"This time (aggressive optimization): {total_pairs:,} pairs")
    ...
    if skipped_pairs:
        print(f"⚠️  Warning: {len(skipped_pairs)} cluster pairs were skipped due to exceeding limits")
    ...
    print(f"  3. python3 inter_ie_enhanced_extractor_fixed.py")
