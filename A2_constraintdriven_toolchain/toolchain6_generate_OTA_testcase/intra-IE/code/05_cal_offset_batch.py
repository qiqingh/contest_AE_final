...
INPUT_DIR = "../output/04_reencode"
OUTPUT_DIR = "../output/05_calOffset"
...
def process_file(input_file_path, original_hex, hex_stream1, paths):
    ...
    # Read hex_stream2 from file
    hex_stream2 = read_hex_from_file(input_file_path)
    if not hex_stream2:
        return False
    
    # Process hex replacement
...
def main():
    ...
    # Ensure output and diagnostic directories exist
    ...
    with open(DIAGNOSIS_FILE, 'w') as diag_file:
        diag_file.write(f"[{datetime.now()}] Starting batch processing of Hex replacement offset calculation\n")
        diag_file.write(f"Input directory: {INPUT_DIR}\n")
        diag_file.write(f"Output directory: {OUTPUT_DIR}\n\n")
    ...
    with open(DIAGNOSIS_FILE, 'a') as diag_file:
        diag_file.write(f"Found {total_files} files to process\n\n")
    ...
    # Print and Record Summary
    summary = f"\n[{datetime.now()}] Processing completed\n"
    summary += f"Successfully processed: {success_count}/{total_files} files\n"
