...
FLATTENED_DIR = "../../../toolchain5_dsl_to_testcase/output/test_cases_inter_ie"
...
def reconstruct_from_metadata(metadata, current_path=""):
    ...
    metadata is a dict where the keys are the complete paths of each node and the values are the metadata information for that node.
    If there is a "structure_type" in metadata[current_path], it indicates that the node is a container.
    Otherwise, treat it as a leaf node and return None (which will be populated by flattened records later).
    """
...
def set_nested_value(nested, field_path, value):
    """
    Insert the value into the nested object according to field_path.
...
def overlay_flattened(skeleton, flattened_records, log_file):
    ...
    Cover the corresponding positions in the skeleton.
    """
    errors = []
...
def process_file(flattened_file, metadata, log_file):
    """Process a single flattened JSON file"""
    ...
    try:
        ...
        if isinstance(flattened_records, list):
            ...
            if len(flattened_records) > 0:
                log_both(log_file, f"First element type: {type(flattened_records[0]).__name__}")
                ...
                if isinstance(flattened_records[0], dict):
                    ...
                    log_both(log_file, f"First element's key: {keys}")
                    ...
                    log_both(log_file, f"Field check:")
    ...
    log_both(log_file, "[Step 2] Reconstructing structural framework based on metadata...")
    try:
    ...
    log_both(log_file, "[Step 3] Overlay flattened records onto skeleton...")
    try:
...
def main():
    ...
    with open(DIAGNOSIS_FILE, 'w') as log_file:
        ...
        # ========== Loading Metadata ==========
        log_both(log_file, "="*80)
        log_both(log_file, "[Initialization] Loading metadata file...")
        log_both(log_file, "="*80)
        ...
        log_both(log_file, "\n" + "="*80)
        log_both(log_file, f"Detailed log has been saved to: {DIAGNOSIS_FILE}")
