...
def fix_bitstrings(obj):
    """
    If the JSON contains strings in the format "010101...", convert them to byte data using bitstr_to_bytes.
    """
...
def restore_tuples(obj):
    """
    Convert lists like ["setup", {...}] back to tuples of the form ("setup", {...}).
    """
...
def recursive_convert_bitstring_lists(obj):
    ...
    If encountering a list with a length of 2 where the second element is an int less than 100 (typically the bit length of a BIT STRING does not exceed 100),
...
def reencode_from_json(json_file, field_name="DL_CCCH_Message", log_file=None):
    ...
    try:
        ...
        # 2. First process bit string strings that may be saved as "010101..."
        ...
        # 5. Get ASN.1 Structure Definition
        try:
        ...
        try:
            # ===== Add debug information - before set_val =====
            if log_file:
                ...
                # If there are constraints, check scalability
                if hasattr(sch, '_const_val') and sch._const_val:
                    ...
                    debug_msg += f"[DEBUG] Constraint definition not found\n"
            ...
            # ===== Optional: Check again after set_val =====
            ...
            error_msg = f"[ERROR] Error occurred while restoring ASN.1 object: {e}"
            if log_file:
                log_file.write(error_msg + "\n")
            print(error_msg, flush=True)
            return None
        ...
        for json_file in json_files:
            ...
            if encoded_bytes:
                ...
                try:
                    ...
                    success_msg = f"[INFO] Encoding result has been saved to {output_file}"
                    log_file.write(success_msg + "\n")
                    print(success_msg, flush=True)
                    success_count += 1
                except Exception as e:
                    error_msg = f"[ERROR] Error occurred while saving results to file {output_file}: {e}"
                    log_file.write(error_msg + "\n")
                    print(error_msg, flush=True)
        
        # Record Processing Summary
        summary = f"[{datetime.now()}] Processing completed"
